...
$(window).load(function() {
    ...
    video.find('video').css({'width': width});
    ...
    socket.on('local-candidate-broadcast', function(candidate) {
        ...
        remotePeerConnection.addIceCandidate(new RTCIceCandidate({sdpMLineIndex: candidate.sdpMLineIndex, candidate: candidate.candidate}));

    });

    socket.on('remote-candidate-broadcast', function(candidate) {
        console.log('Inside remote-candidate-broadcast');
        localPeerConnection.addIceCandidate(new RTCIceCandidate({sdpMLineIndex: candidate.sdpMLineIndex, candidate: candidate.candidate}));
    });
    ...
    function resizeUserPrompt() {
        ...
        userPrompt.css({'position': 'absolute', 'top': top, 'left': left});
...
function setToolbar() {
    ...
    $('#toolbar').css({'position': 'relative', 'top': icontop, 'width': 'initial'});
}
...
function changeToolbar() {
    ...
    if (!ESC_BUTTON_PRESSED && isChrome) {
        if ((document.webkitIsFullScreen) && !FULL_SCREEN_BUTTON_CLICKED) {
            ...
            $('#toolbar').css({'position': 'relative', 'top': icontop, 'width': 'initial'}).find('div#icons-tools').width(fff);
...
function resetLocalShareScreenDiv() {
    $('#screen-share').css({'position': 'initial', 'height': 'initial', 'width': 'initial', 'left': 'initial', 'top': 'initial'});

    $('#screen-share').find('#my-screen-share').css({'display': 'initial', 'position': 'initial', 'height': 'initial', 'width': 'initial', 'z-index': 'initial'});
}
